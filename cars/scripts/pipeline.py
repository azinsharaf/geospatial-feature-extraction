...
def main():
    ap = argparse.ArgumentParser(description="Starter end-to-end car-detection pipeline (YOLOv8).")
    ap.add_argument("step", choices=["ingest","prepare","train","infer","export","visualize","all"], help="Step to execute")
    args = ap.parse_args()
    if args.step == "ingest":
        ingest_data()
